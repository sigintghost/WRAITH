...
def setup_integrations():
    ...
    if ans == 'y':
        from modules.integrations.snowflake_connector import setup_snowflake
        setup_snowflake()
    ans = input('  Setup WebCTRL PostgreSQL? [y/N]: ').strip().lower()
    if ans == 'y':
        from modules.integrations.pg_connector import setup_pg
        setup_pg()
    ans = input('  Setup CMMS work orders? [y/N]: ').strip().lower()
    if ans == 'y':
        from modules.integrations.workorder_agent import setup_workorder
        setup_workorder()
